...
config_path = '/home1/lujingyu/projects/AI4Science/project/configs/data/contton_sub.yaml'
model_scale = 'yolo11m'
model = YOLO(f'{model_scale}-seg.yaml', task='segment').load(f"{model_scale}-seg.pt")  # build from YAML and transfer weights

# Train the model
imgsz = 1024
results = model.train(data=config_path, epochs=200, imgsz=imgsz, batch=4)
