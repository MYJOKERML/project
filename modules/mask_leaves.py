...
def save_masked_image_with_meta_info(img_path: str, masks: Masks, boxes: Boxes, conf: torch.Tensor, save_root: str):
    img = Image.open(img_path).convert('RGB')
    img = np.array(img)
    # 储存元信息
    meta_info = {
        'ori_image_path': img_path,
        'mask_image_path': os.path.join(save_root, 'masked_images', f'{os.path.basename(img_path)}'),
        'height': img.shape[0],
        'width': img.shape[1],
...
if __name__=='__main__':
    ...
    for i, result in enumerate(results):
        ...
        conf = conf[valid_idx]
        # 保存掩码图像和元信息
        save_masked_image_with_meta_info(img_path, masks, boxes, conf, save_root)
